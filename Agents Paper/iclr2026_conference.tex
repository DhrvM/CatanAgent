...
\texttt{dhruvm6@illinois.edu} \quad
\texttt{rjuare8@illinois.edu} \\
\texttt{sjanga2@illinois.edu} \quad
\texttt{vayung2@illinois.edu}
}
...
CatanAgents is a decentralized cooperative system of four specialized GPT-4o agents: the \textbf{Strategy} orchestrator and the \textbf{Risk}, \textbf{Development}, and \textbf{Trading} executors. All persistent state flows through a thread-safe \emph{Scratchpad}, and inter-agent control flow is restricted to the topology in Figure~\ref{fig:arch}, enforced at runtime by an \texttt{ALLOWED\_CHANNELS} whitelist in \texttt{BaseAgent}. Per turn, Strategy polls the socket, writes state to the Scratchpad, invokes Risk for deterministic probability analysis, asks GPT-4o for a structured plan, and delegates tactical phases to Trading and Development.

\begin{figure}[t]
\centering
\begin{tikzpicture}[
    every node/.style={draw, rounded corners, minimum width=1.9cm, minimum height=0.6cm, align=center, font=\scriptsize},
    edge/.style={<->, thick, >=Latex}
]
  \node (S) at (0, 1.0) {Strategy};
  \node (R) at (-3.0, 0) {Risk};
  \node (D) at (3.0, 0) {Development};
  \node (T) at (0, -1.0) {Trading};
  \draw[edge] (S) -- (R) node[midway, above, draw=none, font=\scriptsize]{analyze / consult};
  \draw[edge] (S) -- (D) node[midway, above, draw=none, font=\scriptsize]{build / discard / robber};
  \draw[edge] (S) -- (T) node[midway, right, draw=none, font=\scriptsize]{trade policy};
  \draw[edge] (D) -- (R) node[midway, below, draw=none, font=\scriptsize]{EV / robber targets};
\end{tikzpicture}
...
All probability-critical decisions use a deterministic backend. For a vertex $v$ with adjacent hexes $\text{adj}(v)$, expected per-turn production is
\begin{equation}
E[\text{income}_v] = \sum_{H \in \text{adj}(v)} \tfrac{\mathrm{pips}(H)}{36} \cdot m \cdot \mathbf{1}[H \neq \text{desert, robber}],
\label{eq:income}
\end{equation}
with $m=2$ for cities and $m=1$ for settlements. Opponent threat score is
$\textit{score} = 10\,\mathrm{VP} + 8\,r + 3\,d + 0.5\,c + b_{\textit{road}} + b_{\textit{army}} + 5\,a$,
where $r$ is income, $d$ is dev cards, $c$ is total cards, $a$ is achievements, and $b_{\textit{road}}, b_{\textit{army}}$ are proximity bonuses; threats are \emph{critical} when $\mathrm{VP}\geq 8$ or $\textit{score}\geq 75$. Settlement placements use a scarcity-weighted variant of Eq.~\ref{eq:income}, replacing $m$ with a weight $w(\textit{res}(H))\in\{2.0, 1.5, 1.0, 0.6\}$ for hand counts $\{0, 1, 2$--$3, 4+\}$, plus a small diversity bonus for unseen resources. Trade evaluation produces a score in $[0,1]$ starting at $0.5$ with $+0.25$ per desperately-needed resource received and $-0.20$ per such resource given; opponents at $\mathrm{VP}\geq 8$ incur $-0.35$. Counter-offers are built automatically in the near-threshold window $[\textit{min\_accept}\!-\!0.2, \textit{min\_accept}]$. A per-player win probability combines VP, income, dev cards, and achievement proximity with weights $(0.50, 0.20, 0.15, 0.15)$, normalized across players.

\subsection{Prompting, Errors, and Trade-offs}
\label{sec:prompts}

Prompts live in per-agent \texttt{prompts.py} files and are composed from Scratchpad views. Strategy's system prompt demands a single \texttt{StrategyPlan} JSON with no other text; Risk uses three prompts (consultant, plan-assessor, narrative); Development and Trading use task-scoped prompts with disjoint tool subsets. Tool-calling follows a bounded ReAct pattern (2 steps for discard/robber, 6 for proactive trading, 2 for reactive); malformed tool calls trigger a ``repair'' message until the cap, after which the deterministic fallback runs. Error handling is layered: OpenAI 429s use exponential backoff up to 90\,s; illegal server actions are skipped rather than re-planned; the \texttt{threading.Lock} prevents write interleaving; and outgoing offers are capped by a 60\,s polling loop that auto-cancels and falls back to a bank trade. We rejected three alternatives for principled reasons: a single ReAct agent suffers from prompt overload (the failure mode we observed in early prototypes); a fully LLM-driven pipeline (Risk doing math) was rejected because probability-critical quantities must be exact; and unconstrained multi-agent chat was rejected because it lets Trading gather data that contradicts Strategy. We pay for this in higher per-turn LLM usage, in exchange for legality, interpretability, and replayability.
...
Full-game evaluation is still in progress at the time of submission. Preliminary runs confirm that the system-level plumbing works end-to-end: the setup-phase heuristic produces legal opening placements on every seed we have tested, the scratchpad-based turn loop executes without deadlocks under concurrent socket updates, and Risk's deterministic functions return consistent expected-income and threat scores on saved game states. The full cross-agent comparison against the ReAct and rule-based baselines is pending completion of the Development agent's build-queue executor, which is the last module blocking end-to-end games.

Table~\ref{tab:results} shows the headline comparison we will populate in the final version of this paper. Numbers are intentionally left as TBD; we report the table schema here so that our evaluation plan is concrete and reproducible.

\begin{table}[h]
\caption{Planned headline comparison. Final numbers are pending full-game evaluation.}
\label{tab:results}
\begin{center}
\small
\begin{tabular}{@{}lccccc@{}}
\toprule
\textbf{Agent} & \textbf{Win rate} & \textbf{Avg VP} & \textbf{Rounds / win} & \textbf{Task score} & \textbf{Illegal move \%} \\
\midrule
Single-Prompt ReAct       & TBD & TBD & TBD & TBD & TBD \\
Rule-Based LLM            & TBD & TBD & TBD & TBD & TBD \\
CatanAgents (ours)        & TBD & TBD & TBD & TBD & TBD \\
\bottomrule
\end{tabular}
\end{center}
\end{table}
