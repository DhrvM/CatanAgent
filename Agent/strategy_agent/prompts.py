...
3. A Risk consultation (competitive analysis — who's winning, threats)
4. A Development consultation (building advice — what and where to build)
5. Your previous strategy plan (for continuity)
6. Messages from other agents (trade results, build results)
...
- should_trade_first: true if trading should happen before building, false otherwise
...
- Use the Development consultation to inform your build queue.
...
- Set should_trade_first to true if you need resources before building, false if you can build now.
...
def build_strategy_context(
    ...
    # 3. Risk consultation (competitive intelligence)
    if risk_consultation:
        sections.append("\n## Risk Consultant's Assessment")
        sections.append(risk_consultation)
    
    # 4. Development consultation (building advice)
    if dev_consultation:
        sections.append("\n## Development Consultant's Advice")
        sections.append(dev_consultation)
    ...
    sections.append(
        ...
        "Use the Risk and Development consultations to inform your decisions. "
        "Include all fields: long_term_goal, short_term_goals, priority_resources, "
        "build_queue, trade_policy, should_trade_first, risk_tolerance, reasoning."
    )
