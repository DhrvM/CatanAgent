...
Task quality is supported by deterministic task specifications. Each of the 20 tasks takes a deterministic input and a scoring function. Production weights, resource diversity, expansion access, port synergy, build-plan values, and resource need/surplus are reused consistently across tasks. For long-term tasks, we moved from immediate scoring to delayed four-to-six-turn simulation.
...
\begin{itemize}
    ...
    \item \texttt{settlementTargetValue} is the best open settlement value at either endpoint of the road, using the same production-diversity-port mixture as \texttt{reachableValue}. Endpoints occupied by a building or invalidated by the distance rule receive $0$.
