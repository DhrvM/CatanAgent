...
// ============================================================================
// EXPECTED RESOURCE GAINS (NEXT DICE ROLL)
// ============================================================================

/** 2d6 probability mass function (includes 7, which yields zero distribution) */
function get2d6Probabilities() {
  // counts out of 36
  const counts = {
    2: 1, 3: 2, 4: 3, 5: 4, 6: 5,
    7: 6,
  ...
  const probs = {};
  for (const [total, c] of Object.entries(counts)) {
    probs[Number(total)] = c / 36;
  }
  return probs;
}

/**
 * Pure (non-mutating) version of distributeResources:
 * returns per-player resource deltas for a given roll total.
 *
 * IMPORTANT: Mirrors distributeResources() behavior:
 * - Only hexes with matching number
 * - Skips robber hex
 * - Awards 1 per settlement, 2 per city
 * - Uses findBuildingAtHexVertex() (handles equivalent vertex keys)
 * - Dedupes per (hex, physical_vertex_found_key) to prevent double counting
 */
function computeDistributionGains(game, roll) {
  const gains = {};
  game.players.forEach((_, idx) => {
    gains[idx] = { brick: 0, lumber: 0, wool: 0, grain: 0, ore: 0 };
  });

  // No distribution on 7
  if (roll === 7) return gains;

  const processedHexVertices = new Set();

  Object.entries(game.hexes).forEach(([hKey, hex]) => {
    if (hex.number === roll && hKey !== game.robber) {
      // For each of 6 vertices around this hex
      for (let dir = 0; dir < 6; dir++) {
        const buildingInfo = findBuildingAtHexVertex(game, hex.q, hex.r, dir);
...
/**
 * Get per-player expected resource gains on the *next* dice roll.
 *
 * Returns:
 * {
 *   expected: { [playerIndex]: {brick,lumber,wool,grain,ore,total} },
 *   rollProbabilities: {2..12},
 * }
 */
export function getExpectedNextRollGains(game) {
  const probs = get2d6Probabilities();

  // Initialize expected values
  const expected = {};
  game.players.forEach((_, idx) => {
    expected[idx] = { brick: 0, lumber: 0, wool: 0, grain: 0, ore: 0, total: 0 };
  });

  // Sum over all totals 2..12 (7 contributes zero anyway)
  for (let roll = 2; roll <= 12; roll++) {
    const p = probs[roll] || 0;
    if (p === 0) continue;

    const gains = computeDistributionGains(game, roll);

    for (const [pIdxStr, g] of Object.entries(gains)) {
      const pIdx = Number(pIdxStr);
      expected[pIdx].brick += p * g.brick;
      expected[pIdx].lumber += p * g.lumber;
      expected[pIdx].wool += p * g.wool;
      expected[pIdx].grain += p * g.grain;
      expected[pIdx].ore += p * g.ore;
    }
  }

  // Add totals
  for (const pIdxStr of Object.keys(expected)) {
    const pIdx = Number(pIdxStr);
    const e = expected[pIdx];
    e.total = e.brick + e.lumber + e.wool + e.grain + e.ore;
  }

  return { expected, rollProbabilities: probs };
}

/**
 * Compute value of moving robber to a specific hex 
 * (only in terms of expected resource gain in future, not immediate gain from steal)
 *
 * Returns:
 * {
 *   deltaPerPlayer: { playerIndex: {brick,lumber,wool,grain,ore,total} },
 *   myDelta: number,
 *   opponentDelta: number,
 *   totalSwing: number
 * }
 */
export function getRobberMoveValue(game, targetHexKey, playerId) {
  const playerIndex = game.players.findIndex(p => p.id === playerId);
  if (playerIndex === -1) {
    return { error: 'Player not found' };
  }

  if (!game.hexes[targetHexKey]) {
    return { error: 'Invalid hex' };
  }

  const originalRobber = game.robber;

  // Compute baseline expected gains
  const before = getExpectedNextRollGains(game).expected;

  // Move robber temporarily
  game.robber = targetHexKey;

  const after = getExpectedNextRollGains(game).expected;

  // Restore
  game.robber = originalRobber;

  const deltaPerPlayer = {};
  let myDelta = 0;
  let opponentDelta = 0;

  for (const [idxStr, afterVals] of Object.entries(after)) {
    const idx = Number(idxStr);
    const beforeVals = before[idx];

    deltaPerPlayer[idx] = {
      brick: afterVals.brick - beforeVals.brick,
      lumber: afterVals.lumber - beforeVals.lumber,
      wool: afterVals.wool - beforeVals.wool,
      grain: afterVals.grain - beforeVals.grain,
      ore: afterVals.ore - beforeVals.ore,
      total: afterVals.total - beforeVals.total
    };

    if (idx === playerIndex) {
      myDelta += deltaPerPlayer[idx].total;
    } else {
      opponentDelta += deltaPerPlayer[idx].total;
    }
  }
  ...
  return {
    ...
    myDelta,
    opponentDelta,
    totalSwing: myDelta - opponentDelta
  };
}

/**
 * Compute value of placing a settlement at a vertex.
 *
 * options:
 * {
 *   ignoreLegality?: boolean,
 *   ignoreResources?: boolean
 * }
 *
 * Returns:
 * {
 *   delta: {brick,lumber,wool,grain,ore,total},
 *   valid: boolean,
 *   error?: string
 * }
 */
export function getSettlementValue(game, playerId, vKey, options = {}) {
  const playerIndex = game.players.findIndex(p => p.id === playerId);
  if (playerIndex === -1) {
    return { valid: false, error: 'Player not found' };
  }

  const { ignoreLegality = false } = options;

  if (!ignoreLegality) {
    const validation = canPlaceSettlement(game, playerId, vKey, false);
    if (!validation.valid) {
      return { valid: false, error: validation.error };
    }
  }

  const before = getExpectedNextRollGains(game).expected[playerIndex];

  const originalVertex = game.vertices[vKey];
  const originalSettlements = game.players[playerIndex].settlements;
  const originalVP = game.players[playerIndex].victoryPoints;

  // Place settlement temporarily
  game.vertices[vKey] = { building: 'settlement', owner: playerIndex };
  game.players[playerIndex].settlements--;
  game.players[playerIndex].victoryPoints++;

  const after = getExpectedNextRollGains(game).expected[playerIndex];

  // Restore state
  game.vertices[vKey] = originalVertex;
  game.players[playerIndex].settlements = originalSettlements;
  game.players[playerIndex].victoryPoints = originalVP;

  const delta = {
    brick: after.brick - before.brick,
    lumber: after.lumber - before.lumber,
